...
function updateQuotaDisplay() {
  ...
  quotaEl.textContent = `(${myUsed}/${myQuota} séances cette semaine)`;
  const quotaReached = myUsed >= myQuota;
  quotaEl.style.color = quotaReached ? "var(--red)" : "";
  if (quotaReached && !isAdmin) {
    quotaBanner.textContent = `Vous avez atteint votre nombre de séances pour cette semaine (${myQuota}). Seul l'administrateur peut ajouter une séance supplémentaire.`;
...
function buildSlotCell(dayIdx, hour) {
  ...
  const quotaReached = myQuota !== null && myUsed >= myQuota;
...
async function book(dayIdx, hour) {
  try {
    ...
    await loadWeek();
    showToast(`✓ Réservation confirmée — ${DAYS[dayIdx]} ${hour}h-${hour + 1}h`);
  } catch (e) {
    showError(e.message);
  }
}
